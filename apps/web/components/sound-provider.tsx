...
export function SoundProvider({ children }: { children: React.ReactNode }) {
  ...
  return <SoundContext.Provider value={value}>{children}<button type="button" onClick={value.toggle} aria-label={enabled ? "Silenciar sonidos" : "Activar sonidos"} aria-pressed={!enabled} title={enabled ? "Silenciar sonidos" : "Activar sonidos"} className="fixed bottom-4 left-4 z-[60] grid h-11 w-11 place-items-center rounded-full border border-white/10 bg-slate-950/80 text-slate-300 shadow-xl backdrop-blur-md transition hover:border-emerald-400/40 hover:text-emerald-300 focus-visible:outline-2 focus-visible:outline-emerald-400">{enabled ? "♫" : "♪̸"}</button></SoundContext.Provider>;
}
